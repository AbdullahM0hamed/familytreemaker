        ...
        for p in gen:
            ...
            for i in range(0, int(l/2)):
                ...
                if self.outmode == 0:
                    ...
                    if h.parents[0] == h.parents[1]:
                        # yield '\t\t%s;' % (p.id, h.id)
                        yield '\t\t%s;' % (p.id)
                    else:
                        yield '%s -> h%d -> %s' % (h.parents[0], h.id, h.parents[1]) + \
                            '\t\th%d%s;' % (h.id, Family.invisible)
